...
pub fn query_credits(
    ...
    if let Some(claw) = data.get("claw_pass_quota") {
        result["claw_pass_quota"] = json!({
            "has_pass": claw.get("has_pass").and_then(|v| v.as_bool()).unwrap_or(false),
            "package_id": claw.get("package_id").and_then(|v| v.as_str()).unwrap_or(""),
            "tier": claw.get("tier").and_then(|v| v.as_str()).unwrap_or(""),
            "daily_quota_seconds": claw.get("daily_quota_seconds").and_then(|v| v.as_i64()).unwrap_or(0),
            "used_seconds": claw.get("used_seconds").and_then(|v| v.as_i64()).unwrap_or(0),
            "remain_seconds": claw.get("remain_seconds").and_then(|v| v.as_i64()).unwrap_or(0),
            "cycle_start_at": claw.get("cycle_start_at").and_then(|v| v.as_str()).unwrap_or(""),
            "cycle_end_at": claw.get("cycle_end_at").and_then(|v| v.as_str()).unwrap_or(""),
            "next_refresh_at": claw.get("next_refresh_at").and_then(|v| v.as_str()).unwrap_or(""),
            "refresh_timezone": claw.get("refresh_timezone").and_then(|v| v.as_str()).unwrap_or(""),
        });
    }
