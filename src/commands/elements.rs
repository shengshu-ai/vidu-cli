...
pub fn preprocess(name: &str, elem_type: &str, images: &[String]) {
    ...
    let components: Vec<Value> = images
        .iter()
        .enumerate()
        .map(|(i, img)| {
            json!({
...
pub fn create(
    ...
    if let Some(s) = style {
        if s.chars().count() > 64 {
            client::fail(
                "client_error",
                "recaption.style must not exceed 64 characters",
                None,
                None,
                None,
            );
    ...
    if let Some(d) = description {
        ...
        if d.is_empty() || char_count > 1280 {
            client::fail(
                "client_error",
                "recaption.description must be 1-1280 characters",
                None,
                None,
                None,
            );
    ...
    let processed_images: Vec<String> = images
        .iter()
        .map(|img| crate::commands::tasks::process_image_input(img))
        .collect();

    // Step 3: Preprocess
    let components: Vec<Value> = processed_images
        .iter()
        .enumerate()
        .map(|(i, img)| {
            json!({
    ...
    let prep_style = prep_data
        .get("recaption")
        .and_then(|r| r.get("style"))
        .and_then(|v| v.as_str())
        .unwrap_or("");

    let prep_desc = prep_data
        .get("recaption")
        .and_then(|r| r.get("description"))
        .and_then(|v| v.as_str())
        .unwrap_or("");
...
pub fn list_elements(keyword: Option<&str>, page: i64, pagesz: i64) {
    ...
    let data = client::request_json(
        "GET",
        &format!("{}/vidu/v1/material/elements/personal", base),
        None,
        None,
        Some(&params),
    );
...
pub fn search(keyword: &str, pagesz: i64, sort_by: &str, page_token: &str) {
    ...
    let data = client::request_json(
        "GET",
        &format!("{}/vidu/v1/material/share_elements/feed", base),
        None,
        None,
        Some(&params),
    );
    let share_elements = data
        .get("share_elements")
        .and_then(|v| v.as_array())
        .cloned()
        .unwrap_or_default();
